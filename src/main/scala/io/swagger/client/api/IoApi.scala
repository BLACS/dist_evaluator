...
case class LocatedCellList(l:List[LocatedCell])
...
class IoApi(val defBasePath: String = "http://172.17.0.2:8080",
   ...
   */
  def hashSheet(sheet: String, hash: String): Option[LocatedCellList] = {
    ...
    try {
      apiInvoker.invokeApi(basePath, path, "GET", queryParams.toMap, formParams.toMap, postBody, headerParams.toMap, contentType) match {
        case s: String => {
          val l =
            ApiInvoker.deserialize(s, "array", classOf[LocatedCell]).asInstanceOf[List[LocatedCell]]
          Some(LocatedCellList(l))
          }
        case _ => None
      }
    } catch {
